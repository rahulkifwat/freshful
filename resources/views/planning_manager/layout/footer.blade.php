...
$(document).on('change', '.change_status', function() {
    ...
    $.post("{{ url('planning_manager/ajax/change-status') }}", {_token:'{{ csrf_token() }}', table:table, id:id, status:val},
...
$(document).on('click', '.deleteRecord', function() {
    ...
    $.post("{{ url('planning_manager/ajax/delete-record') }}", {_token:'{{ csrf_token() }}', table:table, id:id},
...
$(document).on('change', '.change_order_status', function() {
    ...
    $.post("{{ url('planning_manager/ajax/change-order-status') }}", {_token:'{{ csrf_token() }}', order_id:id, status:val},
        function(r){ if(r.result) toastr.success(r.message); else toastr.error(r.message); }, 'json');
});
</script>
@stack('scripts')
