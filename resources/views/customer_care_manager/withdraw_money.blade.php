@extends('customer_care_manager.layout.master')
@section('content')
<div class="right-panel">
    <div class="content">
        <div class="container-fluid">
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col-auto">
                        <h3 class="page-title">Withdraw Money Requests</h3>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('customer_care_manager.dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active">Withdraw Money</li>
                        </ul>
                    </div>
                </div>
            </div>

            @if(session('success'))
                <div class="alert alert-success">{{ session('success') }}</div>
            @endif

            <div class="card">
                <div class="card-header"><h4 class="card-title">All Withdraw Requests</h4></div>
                <div class="card-body">
                    @if($rows instanceof \Illuminate\Support\Collection && $rows->isEmpty() || (method_exists($rows, 'total') && $rows->total() == 0))
                        <div class="alert alert-info">No withdraw requests found.</div>
                    @else
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>Buyer Name</th>
                                        <th>Amount (₹)</th>
                                        <th>Status</th>
                                        <th>Date</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($rows as $row)
                                    <tr>
                                        <td>{{ $row->id }}</td>
                                        <td>{{ $row->buyer_name }}</td>
                                        <td>₹{{ number_format($row->amount ?? 0, 2) }}</td>
                                        <td>
                                            <select class="form-control form-control-sm change_status"
                                                data-table="withdraw_amount" data-id="{{ $row->id }}">
                                                @foreach(['pending','approved','rejected'] as $s)
                                                    <option value="{{ $s }}" {{ ($row->status ?? '') == $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>{{ $row->created_at ?? '—' }}</td>
                                        <td>
                                            <button class="btn btn-sm btn-danger deleteRecord"
                                                data-table="withdraw_amount" data-id="{{ $row->id }}">
                                                <i class="fa fa-trash"></i>
                                            </button>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                        </div>
                        @if(method_exists($rows, 'links'))
                            {{ $rows->appends(request()->query())->links() }}
                        @endif
                    @endif
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
